...
class BertIntermediate(nn.Module):
    def __init__(self, config, is_stitched=False):
        super().__init__()
        if is_stitched:
            self.dense = nn.Linear(config.stitch_hidden_size, config.stitch_intermediate_size)
        else:
...
class StitchedBertModel(BertPreTrainedModel):
    """
    ...
    def get_input_embeddings(self):
        return (self.embeddings1.word_embeddings, self.embeddings2.word_embeddings)

    def set_input_embeddings(self, values):
        self.embeddings1.word_embeddings, self.embeddings2.word_embeddings = values
